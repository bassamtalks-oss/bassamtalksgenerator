  ...
  return (
    <div className={cn('space-y-2', className)}>
      ...
      <div className="flex flex-wrap gap-1.5">
        {options.map(opt => {
          const preview = previewData[opt.id];
          ...
          if (preview) {
            return (
              <HoverCard key={opt.id} openDelay={300} closeDelay={0}>
                <HoverCardTrigger asChild>{button}</HoverCardTrigger>
                <HoverCardContent side="top" className="w-52 p-2 space-y-2">
                  {preview.image && (
                    <img
                      src={preview.image}
                      alt={opt.name}
                  ...
                  {preview.description && (
                    <p className="text-[11px] leading-relaxed text-muted-foreground">{preview.description}</p>
                  )}
                </HoverCardContent>
              </HoverCard>
            );
          ...
          return <React.Fragment key={opt.id}>{button}</React.Fragment>;
        })}
      </div>
    </div>
  );
